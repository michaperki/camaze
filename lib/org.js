...
// Returns { assignment, reassigned } — `reassigned` is true when this entity
// already had an assignment (so the caller, api/org.js, can tell an audit
// log "created" from a "changed" without duplicating this existence check).
async function assignEntity(userId, { provider, scope, entity_id: entityId, department_id: departmentId, person_id: personId }) {
  ...
  const reassigned = existing.length > 0;
  const effectiveFrom = reassigned ? currentMonthStart() : "1970-01-01";
  ...
  return { assignment: rows[0], reassigned };
}
