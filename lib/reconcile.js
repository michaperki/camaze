...
// A single row (fixed key "reconciliation") tracking an in-progress cron
// run that ran out of time budget mid-list (see api/cron/reconcile.js) —
// month plus the last user_id successfully processed, so the next run
// resumes the same month from that point instead of restarting from the
// top or letting the daily month-rotation move on before every user for
// this month has actually been checked. `month`/`last_user_id` are both
// null when nothing is in progress (the normal, caught-up state).
async function getReconciliationCursor() {
  const rows = await rest(`/reconciliation_cursor?key=eq.reconciliation&select=month,last_user_id`);
  const row = rows[0];
  return row ? { month: row.month, lastUserId: row.last_user_id } : null;
}

async function setReconciliationCursor(month, lastUserId) {
  await rest(`/reconciliation_cursor?on_conflict=key`, {
    method: "POST",
    headers: { Prefer: "resolution=merge-duplicates" },
    body: JSON.stringify([{
      key: "reconciliation", month, last_user_id: lastUserId, updated_at: new Date().toISOString(),
    }]),
  });
}

module.exports = {
  reconcileUserMonth, getReconciliationStatus, DRIFT_THRESHOLD_USD,
  getReconciliationCursor, setReconciliationCursor,
};
