...
async function fetchAllCosts(overrides, allowEnvFallback, monthStr) {
  ...
  const settled = await Promise.allSettled(providers.map(p => {
    ...
    return p.fetchCosts(start, end, override);
  }));
...
// scope: the first one in this list it actually has data for. OpenAI is
// pinned to "project" even though its rows now include "api_key" too (see
// providers/openai.js) — project is the more broadly meaningful grouping
// for most orgs; flip the order here if per-key OpenAI display is wanted.
...
// Per-API-key / per-workspace / per-project spend for one calendar month —
...
async function fetchAttribution(overrides, allowEnvFallback, googleProjects, monthStr) {
  ...
  if (overrides.anthropic || allowEnvFallback) {
    try {
      const rows = await anthropicProvider.fetchAttribution(start, end, overrides.anthropic);
      attribution.push(...selectPreferredScope(rows, "anthropic"));
    } catch (e) {
  ...
  if (overrides.openai || allowEnvFallback) {
    try {
      const rows = await openaiProvider.fetchAttribution(start, end, overrides.openai);
      attribution.push(...selectPreferredScope(rows, "openai"));
    } catch (e) {
