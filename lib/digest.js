...
const { resolveOverrides, fetchAllCosts, buildSummary, resolveBudget, resolveFixedCosts, isSubscriptionLine } = require("./costs");
...
async function buildDigestData(userId) {
  ...
  const topModels = costData.dayModels
    .filter(m => m.date === dateStr && m.amount_usd > 0 && !isSubscriptionLine(m.model))
