...
const { resolveOverrides, fetchAllCosts, buildSummary, resolveBudget, resolveFixedCosts, isSubscriptionLine, isNonModelLine } = require("./costs");
...
// Same reformatting rules as public/dashboard.html's prettifyModel — duplicated
...
async function buildDigestData(userId) {
  ...
  const [overrides, budget, fixedCosts] = await Promise.all([
    resolveOverrides(userId),
    resolveBudget(userId),
    resolveFixedCosts(userId, currentMonthStr(businessClock.now())),
  ]);
  ...
  // Same total the dashboard shows for the current month — fixed costs
  // added in full, not extrapolated (see buildSummary in lib/costs.js) —
  // so the email and dashboard can't disagree.
  const summary = buildSummary(costData.days, budget, undefined, undefined, fixedCosts.total);
  ...
  const yesterdayUsage = yesterdayEntry?.usage_usd ?? 0;

  const topModels = costData.dayModels
    .filter(m => m.date === dateStr && m.amount_usd > 0 &&
      !isSubscriptionLine(m.model) && !isNonModelLine(m.provider, m.model))
  ...
  return {
    ...
    yesterdayUsage,
    mtd: summary.mtd,
    usageMtd: summary.usageMtd,
    subscriptionMtd: summary.subscriptionMtd,
...
function renderEmail(data) {
  ...
  const dashboardUrl = appUrl ? `${appUrl}/dashboard.html` : "#";
  ...
  const mtdLabel = data.subscriptionMtd > 0
    ? `${fmtUSD(data.mtd)} — ${fmtUSD(data.usageMtd)} usage, ${fmtUSD(data.subscriptionMtd)} subscriptions`
    : fmtUSD(data.mtd);

  let summaryRows =
    row("Yesterday (usage)", fmtUSD(data.yesterdayUsage)) +
    row("Month to date", mtdLabel) +
...
<body style="margin:0;background:#0d0d0d;font-family:system-ui,-apple-system,'Segoe UI',sans-serif;">
  ...
  let text = `Good morning,\n\nHere's your AI spend summary for ${dateLabel}.\n\n` +
    `Yesterday (usage)   ${fmtUSD(data.yesterdayUsage)}\n` +
    `Month to date       ${mtdLabel}\n` +
