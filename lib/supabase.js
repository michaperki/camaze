...
// { digest_enabled, digest_hour }, or null if the user has no settings row yet.
async function getNotificationSettings(userId) {
  const rows = await rest(`/user_notification_settings?user_id=eq.${encodeURIComponent(userId)}&select=digest_enabled,digest_hour`);
  return rows[0] || null;
}

async function upsertNotificationSettings(userId, digestEnabled, digestHour) {
...
// [{ user_id }] — everyone whose daily digest is due this UTC hour (0-23).
async function listDueDigestUsers(hour) {
  return rest(`/user_notification_settings?digest_enabled=eq.true&digest_hour=eq.${encodeURIComponent(hour)}&select=user_id`);
}
