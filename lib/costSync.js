...
async function syncDailyCostsForProvider(userId, monthStr, providerName, days, dayModels) {
  ...
  const modelRows = dayModels
    ...
    .map(r => ({ date: r.date, model: r.model, amount_usd: r.amount_usd, input_tokens: r.input_tokens ?? null, output_tokens: r.output_tokens ?? null }));
  ...
  const zeroRows = existing
    ...
    .map(r => ({
      ...
      is_subscription: r.is_subscription, amount_usd: 0, input_tokens: null, output_tokens: null, estimated: false, updated_at: now,
    }));
