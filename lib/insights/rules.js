...
function recommend(rows, snapshot, now = new Date(), config = catalog, evidenceNow = now) {
  ...
  for (const row of rows) {
    ...
    const rowTokensKnown = row.input_tokens != null && row.output_tokens != null &&
      Number.isFinite(Number(row.input_tokens)) && Number.isFinite(Number(row.output_tokens));
  ...
  for (const usage of seen.values()) {
    ...
    for (const [, , id] of pairs) {
      ...
      if (!snapshot || !fresh(snapshot.fetchedAt, evidenceNow, 7)) { reason = "Benchmark snapshot missing or older than 7 days"; continue; }
      if (!candidate || candidate.available !== true) { reason = "Candidate availability has not been confirmed"; continue; }
      if (![current, candidate].every(m => fresh(m.reviewedAt, evidenceNow, 30))) { reason = "Provider prices and capabilities need review"; continue; }
