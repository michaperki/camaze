...
async function getSource() {
  if (require('../context').current()) return { status: 'ok', snapshot: require('../simulation/knowledge.json').snapshot };
  return (await rest("/insight_sources?id=eq.artificial-analysis&select=snapshot,last_attempt_at,status,error"))[0] || null; }
