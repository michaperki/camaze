...
//
// Used by the Anthropic per-API-key attribution (providers/anthropic.js) to
// turn token counts into an estimated dollar figure — Anthropic's
// cost_report endpoint has no per-key breakdown, only usage_report/messages
// does, and that one only reports tokens.
...
// A slow GitHub must never block a dashboard response — this is the ceiling
// on how long the attribution fetch (the only caller that awaits this) can
// be stalled by an unresponsive LITELLM_URL before falling back.
...
// `await` it directly; resolvePrice never does — it only ever triggers
...
async function loadLitellmIndex() {
  ...
  litellmLoadPromise = (async () => {
    let cached = null;
    try {
      cached = await timing.mark("price_cache_read", () => getPriceCache(PRICE_CACHE_ID));
    } catch (e) {
    ...
    if (isFresh) {
      ...
      litellmIndex = index;
      console.log(`[pricing] path=fresh-cache — loaded LiteLLM price data from cache (${count} models indexed, fetched ${cached.fetched_at})`);
      return litellmIndex;
    }

    const controller = new AbortController();
    const timeoutId = setTimeout(() => controller.abort(), LITELLM_FETCH_TIMEOUT_MS);
    try {
      const res = await timing.mark("litellm_index_fetch", () => fetch(LITELLM_URL, { signal: controller.signal }));
      if (!res.ok) throw new Error(`HTTP ${res.status} fetching LiteLLM price data`);
      const raw = await res.json();
      const { index, count } = buildLitellmIndex(raw);
      litellmIndex = index;
      console.log(`[pricing] path=fetch — fetched fresh LiteLLM price data (${count} models indexed)`);
      ...
      const reason = e.name === "AbortError" ? `timed out after ${LITELLM_FETCH_TIMEOUT_MS}ms` : e.message;
      if (cached) {
        const { index, count } = buildLitellmIndex(cached.data);
        litellmIndex = index;
        console.warn(
          `[pricing] path=stale-cache — LiteLLM fetch ${reason} — using stale cache from ` +
          `${cached.fetched_at} (${count} models indexed). Stale prices beat no prices.`
        );
        return litellmIndex;
      }
      console.warn(
        `[pricing] path=overrides-only — LiteLLM fetch ${reason} and no price_cache row exists — ` +
...
// attribution fetch) and want current LiteLLM data available before they
// start calling resolvePrice in a loop. A no-op once the index is already
// loaded this process.
function ensurePricesLoaded() {
  if (require("./context").current()) return Promise.resolve();
...
// falls back to overrides alone for the current call; see ensurePricesLoaded
// for callers that can afford to wait.
function resolvePrice(provider, model) {
  if (require("./context").current()) return require("./simulation/scenario").prices[provider]?.[model] || null;
...
// in logs the first time it's hit (the dashboard's attribution fetch),
...
module.exports = { PRICE_PER_MILLION, resolvePrice, warnUnknownModel, ensurePricesLoaded };
