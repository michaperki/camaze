...
function recommend(rows, snapshot, now = new Date(), config = catalog) {
  ...
  for (const usage of seen.values()) {
    ...
    for (const [, , id] of pairs) {
      ...
      if (a.estimated !== false || b.estimated !== false) { reason = "Benchmark score is estimated or its measurement status is unknown"; continue; }
      ...
      if (b.score <= a.score || candidate.input > current.input || candidate.output > current.output || (candidate.input === current.input && candidate.output === current.output)) { reason = "No higher measured score with lower published rates"; continue; }
      insights.push({ current, candidate, benchmark: { name: "Artificial Analysis Intelligence Index", version: a.version, current: a.score, candidate: b.score, currentSource: a.source, candidateSource: b.source, observedAt: a.observedAt, candidateObservedAt: b.observedAt }, usage, fetchedAt: snapshot.fetchedAt });
      reason = null;
    }
