...
function extract(html) {
  ...
  function walk(value) {
    ...
    if (typeof value.id === "string" && typeof value.slug === "string" && Object.hasOwn(value, "intelligenceIndex")) {
      ...
      if (previous && (previous.intelligenceIndex !== value.intelligenceIndex || previous.slug !== value.slug || previous.intelligenceIndexIsEstimated !== value.intelligenceIndexIsEstimated || previous.creator?.slug !== value.creator?.slug)) {
...
function normalize(extracted, catalog, fetchedAt) {
  ...
  return { source: SOURCE_URL, adapter: "aa-public-html-v1", fetchedAt, version: extracted.version, records };
}
...
async function fetchPublicSnapshot(catalog, { fetchImpl = fetch, now = () => new Date(), sleep = ms => new Promise(r => setTimeout(r, ms)) } = {}) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    return normalize(extract(html), catalog, now().toISOString());
  }
}
