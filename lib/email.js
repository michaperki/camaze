...
async function sendEmail(toEmail, subject, html, text) {
  ...
  if (ctx) {
    return require('./simulation/store').rest('/simulation_messages', {
      method: 'POST', headers: require('./context').headers(),
      body: JSON.stringify({ user_id: require('./context').OWNER, business_date: require('./context').now().toISOString(), subject, body: text }),
    });
  }
  if (toEmail.endsWith('@camaze-simulation.invalid')) throw new Error('Simulation email cannot be delivered externally');
