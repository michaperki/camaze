const businessClock = require("./context");
...
async function resolveOverrides(userId) {
  businessClock.assertOwner(userId);
  if (businessClock.current()) return require('./simulation/scenario').connections(businessClock.current().environment);
  const overrides = {};
...
function aggregateProviderResults(resultsByProvider, start, end, { isCurrentMonth, daysInMonth, month } = {}) {
  ...
  for (const p of providers) {
    ...
    for (const { date, model, amount_usd } of providerDayModels || []) {
      if ((!isSubscriptionLine(model) && !isNonModelLine(p.name, model)) || !Number.isFinite(amount_usd)) continue;
    ...
    for (const { date, model, amount_usd, input_tokens, output_tokens } of providerDayModels || []) {
      if (!model || model === UNATTRIBUTED_MODEL || !Number.isFinite(amount_usd)) continue;
      const key = `${date}|${p.name}:${model}`;
      const existing = dayModelTotals.get(key);
      if (existing) {
        existing.amount_usd += amount_usd;
        // Sum only when both sides are known; one unknown poisons the merged
        // row rather than silently under-counting it (see lib/insights/
        // rules.js's own token-completeness reasoning, which this feeds).
        existing.input_tokens = existing.input_tokens != null && input_tokens != null ? existing.input_tokens + input_tokens : null;
        existing.output_tokens = existing.output_tokens != null && output_tokens != null ? existing.output_tokens + output_tokens : null;
      } else {
        dayModelTotals.set(key, { date, provider: p.name, model, amount_usd, input_tokens: input_tokens ?? null, output_tokens: output_tokens ?? null });
      }
    }
  ...
  // Per-provider values are usage-only — subscriptions and Google's
  // non-model GCP charges (Compute Engine, Storage, support, etc.) are both
  // excluded, matching the classification the per-model split (allModels
  // above) uses; usage_usd/subscription_usd carry the same split at the
...
async function fetchProviderAttribution(overrides, allowEnvFallback, monthStr) {
  const { start, end } = resolveMonthRange(monthStr, businessClock.now());
