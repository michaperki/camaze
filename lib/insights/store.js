...
async function getUsage(userId, period, request = rest) {
  ...
  for (let offset = 0; ; offset += 500) {
    const page = await request(`/daily_costs?user_id=eq.${encodeURIComponent(userId)}&date=gte.${period.start}&date=lt.${period.end}&select=date,provider,model,amount_usd,is_subscription,updated_at&order=date,provider,model,is_subscription&limit=500&offset=${offset}`);
    rows.push(...page);
